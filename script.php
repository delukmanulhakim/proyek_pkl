...
<script>
  $(function () {
    ...
    <?php
      $i = 1;
           $query = "SELECT nama FROM tb_dosen";
           $sql_dosen = mysqli_query($con, $query) or die (mysqli_error($con));
               ...
               for ($i=0; $i < count($num); $i++) {
                $c[$i] = $num[$i].','.$nama[$i]; // gabungkan masing" isi array dg (,)
                $c[$i] = explode(',',$c[$i]); // explode/jadikan array berdasarkan pemisah (,)
              }
                 $php_data = json_encode($c,JSON_NUMERIC_CHECK);
                 $php_data = str_replace('"', "'", $php_data);
                
      ?>
      var js_data = <?php echo $php_data."\n";?>
    ...
    //   for (var i = 0; i < js_data.length; i += 0.5) {
    //   x.push([i, js_data['nama']]);
    // }
      ticks: js_data
      }
    })
