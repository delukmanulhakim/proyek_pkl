...
// Instanciation of inherited class
    ...
    $penyimpanan = "../dosen_backend_pengesahan/QRcode/";
    ...
    $link = 'http://localhost/sipesiup/dosen_backend_pengesahan/keabsahan.php?nama='.$nama_dosen.'&tahun='.$tahun.'&matkul='.$nama_ind.'&tanggal='.$tanggal;
...
$no = 1;
$pertemuan = 16;
$query_peserta = mysqli_query($con, "SELECT tb_pesertamatkul.id_klsmatkul, tb_pesertamatkul.nim, tb_mahasiswa.nama FROM tb_pesertamatkul,tb_mahasiswa WHERE tb_pesertamatkul.nim = tb_mahasiswa.nim AND tb_pesertamatkul.id_klsmatkul = '$id_klsmk' ORDER BY nim ASC") or die(mysqli_error($con));
if (mysqli_num_rows($query_peserta) > 0)
{
    while ($data_peserta = mysqli_fetch_array($query_peserta))
    {
        ...
        $pdf->Cell(30,6,$pertemuan,1,0,'C');
        $ket = 'Y';
        $query_hadir = mysqli_query($con, "SELECT * FROM tb_presensi WHERE nim = $nim AND id_klsmatkul='$id_klsmk' AND kehadiran = '$ket'") or die(mysqli_error($con));
